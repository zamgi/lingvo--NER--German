...
using Lingvo.NER.Combined.WebService;
using Lingvo.NER.NeuralNetwork;

namespace Lingvo.NER.Combined.WebServiceTestApp
{
    /// <summary>
    /// 
    /// </summary>
    internal static class Program
    {
        ...
        private static async Task Main( string[] args )
        {
            try
            {
                ...
                var client = new NERCombinedWebServiceClient( httpClient, BASE_URL );
        ...
        private static IEnumerable< string > GetFile_4_NER( CancellationTokenSource cts, bool print2ConsoleTitle, int MAX_FILE_SIZE_IN_BYTES )
        {
            ...
            //
            var paths = new[] { @"..\..\..\..\Lingvo.NER.NeuralNetwork\[resources]\input-text\" };
            //---------------------------------------------------------------//
        ...
        private static async Task Run__NER_1( NERCombinedWebServiceClient client, int MAX_FILE_SIZE_IN_MB )
        {
            var sw = Stopwatch.StartNew();
            ...
            await files.ForEachAsync_( dop, cts.Token, async (fileName, ct) =>
            {
                ...
                try
                {
                    ...
                    var nerCount = result.Words?.Count;
                    CONSOLE.WriteLine( $"processed '{fileName}' => NER: {nerCount.GetValueOrDefault()}" );
                }
                catch ( Exception ex )
                {
