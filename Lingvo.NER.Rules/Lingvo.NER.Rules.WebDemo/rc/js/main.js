$(document).ready(function () {
    ...
    $('#nerStyleCheckbox').prop('checked', !!localStorage.getItem(config.LOCALSTORAGE_KEY + 'nerStyleCheckbox')).change(function () {
        ...
        var $disabled_ner_ss = $(document.styleSheets).filter(function (_, ss) { return (!!ss.href && ss.href.toLowerCase().indexOf(disabled_ner) !== -1); }),
    ...
    (function () {
        ...
        $('#text').val(text).focus();
        $.ajax({ type: 'POST', url: 'ProcessHandler.ashx', data: { text: '_dummy_' } });
    })();
    ...
    $('#mainPageContent').on('click', '#processButton', function () {
        ...
        $.ajax({
            ...
            data: { text: text, reloadModel: reloadModel },
            error: function () { processing_end_with_error('Server error.'); },
            success: function (resp) {
                if (resp.errorMessage) { processing_end_with_error(resp.errorMessage); return; }
