...
namespace System.Collections.Generic
{
    ...
    public class SortedSetEx< T > : ISet< T >, ICollection< T >, ICollection, IReadOnlyCollection< T >
    {
        ...
        #region [.ctor().]
        [M(O.AggressiveInlining)] public SortedSetEx() => Comparer = Comparer< T >.Default;
        [M(O.AggressiveInlining)] public SortedSetEx( IComparer< T > comparer ) => Comparer = comparer ?? Comparer< T >.Default; 
        ...
        public SortedSetEx( IEnumerable< T > collection, IComparer< T > comparer ) : this( comparer )
        {
            ...
            var baseTreeSubSet = collection as TreeSubSet;
        ...
        protected virtual bool InOrderTreeWalk( TreeWalkPredicate< T > action, bool reverse )
        {
            if ( _Root == null )
            {
                return (true);
        ...
        // Return true if the entire tree has been walked. 
        // Otherwise returns false.
        protected virtual bool BreadthFirstTreeWalk( TreeWalkPredicate< T > action )
        {
            if ( _Root == null )
            {
                return (true);
        ...
        public IComparer< T > Comparer { [M(O.AggressiveInlining)] get; }
        #endregion
        ...
        public bool TryGetValue( T item, out T existsItem )
        {
        ...
        protected virtual bool IsWithinRange( T item ) => true;
        #endregion
        ...
        public struct Enumerator : IEnumerator< T >, IEnumerator
        {
            ...
            [M(O.AggressiveInlining)] internal Enumerator( SortedSetEx< T > set )
            {
                _Tree = set;
            ...
            [M(O.AggressiveInlining)] private void Intialize()
            {
                ...
                while ( node != null )
                {
                    var next  = node.Left;  //---(_Reverse ? node.Right : node.Left);
                    var other = node.Right; //---(_Reverse ? node.Left  : node.Right);
        ...
        public struct Enumerator_4_Reverse : IEnumerator< T >, IEnumerator
        {
            ...
            internal Enumerator_4_Reverse( SortedSetEx< T > set )
            {
                _Tree = set;
            ...
            private void Intialize()
            {
                ...
                while ( node != null )
                {
                    var next  = node.Right; //---(_Reverse ? node.Right : node.Left);
                    var other = node.Left;  //---(_Reverse ? node.Left  : node.Right);
                    if ( _Tree.IsWithinRange( node.Item ) )
                    {
                        _Stack.Push( node );
                        node = next;
        ...
        protected virtual Node FindNode( T item )
        {
            var current = _Root;
            while ( current != null )
            {
        ...
        protected virtual int InternalIndexOf( T item )
        {
            var count = 0;
            var current = _Root;
            while ( current != null )
        ...
        protected Node FindRange( T from, T to ) => FindRange( from, to, true, true );
        protected Node FindRange( T from, T to, bool lowerBoundActive, bool upperBoundActive )
        {
            var current = _Root;
            while ( current != null )
            {
                if ( lowerBoundActive && (0 < Comparer.Compare( from, current.Item )) )
                {
                    current = current.Right;
                }
        ...
        /// </summary>
        public static IEqualityComparer< SortedSetEx< T > > CreateSetComparer() => new SortedSetEqualityComparer< T >();
        ...
        /// </summary>                
        public static IEqualityComparer< SortedSetEx< T > > CreateSetComparer( IEqualityComparer< T > memberEqualityComparer ) => new SortedSetEqualityComparer< T >( memberEqualityComparer );
        ...
        private static bool AreComparersEqual( SortedSetEx< T > set1, SortedSetEx< T > set2 ) => set1.Comparer.Equals( set2.Comparer );
        ...
        public void UnionWith( IEnumerable< T > other )
        {
            ...
            {
                var dummy = new SortedSetEx< T >( s, Comparer );
                _Root = dummy._Root;
                Count = dummy.Count;
                /*_version++;*/
                return;
            ...
            if ( (s != null) && (t == null) && AreComparersEqual( this, s ) && ((this.Count >> 1) < s.Count) )
        ...
        private void SymmetricExceptWithSameEC( T[] other, int count )
        {
            ...
            {
                while ( i < count && i != 0 && Comparer.Compare( other[ i ], last ) == 0 )
                {
                    i++;
                }
                if ( i >= count )
        ...
        {
            if ( 0 < Comparer.Compare( lowerValue, upperValue ) ) throw (new ArgumentException( "SR.SortedSet_LowerValueGreaterThanUpperValue", nameof(lowerValue) ));
        ...
        private sealed class TreeSubSet : SortedSetEx< T >
        {
            ...
            protected override bool IsWithinRange( T item )
            {
            ...
            protected override bool InOrderTreeWalk( TreeWalkPredicate< T > action, bool reverse )
            {
                if ( _Root == null )
                {
                    return (true);
            ...
            protected override bool BreadthFirstTreeWalk( TreeWalkPredicate< T > action )
            {
                if ( _Root == null )
                {
                    return (true);
            ...
            protected override Node FindNode( T item )
            {
                if ( !IsWithinRange( item ) )
                {
                    return (null);
            ...
            {
                if ( _LBoundActive && (0 < Comparer.Compare( _Min, lowerValue )) ) throw (new ArgumentOutOfRangeException( nameof(lowerValue) ));
...
#if DEBUG
            protected override void IntersectWithEnumerable( IEnumerable< T > other )
            {
        ...
        /// 
        /// </summary>
        protected delegate bool TreeWalkPredicate< X >( Node node );
        ...
        public sealed class Node
        {
            ...
            [M(O.AggressiveInlining)] public Node( T item )
            {
                // The default color will be red, we never need to create a black node directly.                
                Item  = item;
                IsRed = true;
            }
            [M(O.AggressiveInlining)] public Node( T item, bool isRed )
            {
                // The default color will be red, we never need to create a black node directly.                
                Item  = item;
                IsRed = isRed;
