...
namespace Lingvo.NER.Rules.CustomerNumbers
{
    ...
    /// 
    /// </summary>
    internal struct CustomerNumberValueTuple
    {
        [M(O.AggressiveInlining)] public CustomerNumberValueTuple( CustomerNumberWord _word, in ByTextPreamble_SearchResult _sr )
        {
            word = _word;
            sr   = _sr;
        }
        public CustomerNumberWord          word;
    ...
    /// 
    /// </summary>
    internal readonly struct CustomerNumberValue_SearchResult
    {
        /// <summary>
        /// 
        /// </summary>
        public sealed class Comparer : IComparerByRef< CustomerNumberValue_SearchResult >
        {
            public static Comparer Instance { get; } = new Comparer();
            private Comparer() { }
            public int Compare( in CustomerNumberValue_SearchResult x, in CustomerNumberValue_SearchResult y ) => (y.StartIndex - x.StartIndex);
        }

        [M(O.AggressiveInlining)] public CustomerNumberValue_SearchResult( int startIndex, TextPreambleTypeEnum[] ngrams )
        {
    ...
    /// 
    /// </summary>
    internal static class CustomerNumberValuesMerger
    {
        /// <summary>
        ///
        /// </summary>
        ...
        private struct Finder
        {
            ...
            [M(O.AggressiveInlining)] public static Finder Create() => new Finder() { _Node = _Root };
        ...
        static CustomerNumberValuesMerger()
        {
            var ngrams = Permutator.GetPermutations( new[] { TextPreambleTypeEnum.CustomerNumber } );
            _Root = TreeNode.BuildTree( ngrams );
        }
        #endregion

        #region [.public method's.]
        public static bool TryFindAll( DirectAccessList< CustomerNumberValueTuple > tuples, out IReadOnlyCollection< CustomerNumberValue_SearchResult > results )
        {
            var ss = default(SortedSetByRef< CustomerNumberValue_SearchResult >);
            ...
            for ( int index = 0, len = tuples.Count; index < len; index++ )
            {
                ...
                if ( node.HasNgrams )
                {
                    if ( ss == null ) ss = new SortedSetByRef< CustomerNumberValue_SearchResult >( CustomerNumberValue_SearchResult.Comparer.Instance );
    ...
    /// 
    /// </summary>
    internal static class CustomerNumberValuesMergerExtensions
    {
        [M(O.AggressiveInlining)] public static void AddEx( this SortedSetByRef< CustomerNumberValue_SearchResult > ss, int startIndex, TextPreambleTypeEnum[] ngrams )
        {
            var sr = new CustomerNumberValue_SearchResult( startIndex, ngrams );
