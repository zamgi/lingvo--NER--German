...
namespace Lingvo.NER.Rules.Nationalities
{
    ...
    /// 
    /// </summary>
    public interface INationalitiesModel
    {
        IWordsChainDictionary Nationalities { get; }
        IWordsChainDictionary TextPreambles { get; }
    }
    /// <summary>
    /// 
    /// </summary>
    public sealed class NationalitiesModel : INationalitiesModel
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            public (string Filename, int? Capacity) Nationalities { get; set; }
            public (string Filename, int? Capacity) NationalityPreambles { get; set; }
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            public (StreamReader Sr, int? Capacity) Nationalities { get; set; }
            public (StreamReader Sr, int? Capacity) NationalityPreambles { get; set; }
        }

        #region [.ctor().]
        private WordsChainDictionary _Nationalities;
        ...
        {
            var dashes    = xlat.GetHyphens().Select( h => new word_t() { valueOriginal = h.ToString() } ).ToArray();
        ...
        public NationalitiesModel( in InputParams p )
        {
            var (dashes, vc, tokenizer) = create_environment();
            using ( tokenizer )
            {
                _Nationalities = Init_Nationalities( p.Nationalities       , tokenizer, dashes, vc );
                _TextPreambles = Init_TextPreambles( p.NationalityPreambles, tokenizer );
            }
        }
        public NationalitiesModel( in InputParams_2 p )
        {
            var (dashes, vc, tokenizer) = create_environment();
            using ( tokenizer )
            {
                _Nationalities = Init_Nationalities( p.Nationalities       .Sr, p.Nationalities       .Capacity, tokenizer, dashes, vc );
                _TextPreambles = Init_TextPreambles( p.NationalityPreambles.Sr, p.NationalityPreambles.Capacity, tokenizer );
            }
        }
        #endregion
        ...
        private static WordsChainDictionary Init_Nationalities( in (string Filename, int? Capacity) p, Tokenizer tokenizer, word_t[] dashes, VersionCombiner< word_t > vc )
        {
            using ( var sr = new StreamReader( p.Filename ) )
            {
                return (Init_Nationalities( sr, p.Capacity, tokenizer, dashes, vc ));
            }
        }
        private static WordsChainDictionary Init_Nationalities( StreamReader sr, int? capacity, Tokenizer tokenizer, word_t[] dashes, VersionCombiner< word_t > vc )
        {
            // copy-pasted from NamesModel
        ...
        public IWordsChainDictionary Nationalities { [M(O.AggressiveInlining)] get => _Nationalities; }
