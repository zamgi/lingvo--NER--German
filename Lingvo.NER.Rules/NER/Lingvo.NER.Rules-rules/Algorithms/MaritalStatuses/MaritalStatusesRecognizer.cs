...
namespace Lingvo.NER.Rules.MaritalStatuses
{
    ...
    /// 
    /// </summary>
    internal sealed class MaritalStatusesRecognizer
    {
        public const int MAX_DISTANCE_FROM_PREAMBLE = 3;

        #region [.ctor().]
        private const char SPACE = ' ';
        private IMaritalStatusesModel          _Model;
        private IWordsChainDictionary        _TextPreambles;
        private IWordsChainYesNoDictionary   _YesNoPreambles;
        private MaritalStatusesSearcher      _MaritalStatusesSearcher;
        private StringBuilder                _ValueUpperBuff;
        private StringBuilder                _ValueOriginalBuff;

        public MaritalStatusesRecognizer(IMaritalStatusesModel model )
        {
            _Model = model;

            _TextPreambles = _Model.TextPreambles;
            _YesNoPreambles = _Model.YesNoPreambles;

            _MaritalStatusesSearcher = new MaritalStatusesSearcher(_Model.MaritalStatuses );
        ...
        public void Run(List<word_t> words)
        {
            ...
            for (int index = 0, len = words.Count; index < len; index++)
            {
                ...
                if (_TextPreambles.TryGetFirst(words, index, out var length))
                {
                    ...
                    if (!_MaritalStatusesSearcher.TryFindFirst2Rigth(words, startIndex, MAX_DISTANCE_FROM_PREAMBLE, out sr))
                    {
                        continue;
                    }
                }
                ...
                {
                    int yesNoIndex = index;
                    if(_YesNoPreambles.TryGetFirst(words, ref yesNoIndex, out length))
                    {
                        index = yesNoIndex;
                        sr = new SearchResult(index, length);
                    }
                    else
                    {
                        continue;
                ...
                CreateMaritalStatusWord(words, in sr);
                index = startIndex2 - 1;
                has = true;
            }
        ...
        private void CreateMaritalStatusWord(List<word_t> words, in SearchResult sr)
        {

            var w1 = words[sr.StartIndex];

            var nw = new MaritalStatusWord(w1.startIndex, GetMaritalStatusValue(words, in sr));
        ...
        [M(O.AggressiveInlining)] private string GetMaritalStatusValue( List< word_t > words, in SearchResult sr )
        {
            for ( int i = sr.StartIndex, j = sr.Length; 0 < j; j--, i++ )
            {
                var w = words[ i ];
