...
using Lingvo.NER.NeuralNetwork.WebService;

namespace Lingvo.NER.NeuralNetwork.WebServiceTestApp
{
    /// <summary>
    /// 
    /// </summary>
    internal static class Program
    {
        ...
        private static async Task Main( string[] args )
        {
            try
            {
                ...
                var client = new NERNeuralNetworkWebServiceClient( httpClient, BASE_URL );
        ...
        private static async Task Run__NER_1( NERNeuralNetworkWebServiceClient client, int MAX_FILE_SIZE_IN_MB )
        {
            var sw = Stopwatch.StartNew();
            ...
            await files.ForEachAsync_( dop, cts.Token, async (fileName, ct) =>
            {
                ...
                try
                {
                    ...
                    var nerCount = result.NerResults?.Sum( nr => nr.Tuples.Count( t => t.Ner != "O" ) );
