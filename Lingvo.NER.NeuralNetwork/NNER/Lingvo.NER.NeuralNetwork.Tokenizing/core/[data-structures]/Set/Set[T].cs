...
namespace System.Collections.Generic
{
    ...
    public sealed class Set< T > : ICollection< T >, IReadOnlyCollection< T >
    {
        ...
        #region [.ctor().]
        [M(O.AggressiveInlining), TP(REASON)] public Set( IEqualityComparer< T > comparer ) : this( DEFAULT_CAPACITY, comparer ) { }
        [M(O.AggressiveInlining), TP(REASON)] public Set( int capacity = DEFAULT_CAPACITY, IEqualityComparer< T > comparer = null )
        {
        ...
        [M(O.AggressiveInlining), TP(REASON)] public Set( IList< T > lst, IEqualityComparer< T > comparer = null ) : this( lst, lst.Count, comparer ) { }
        [M(O.AggressiveInlining), TP(REASON)] public Set( IEnumerable< T > seq, int? capacity = null, IEqualityComparer< T > comparer = null ) 
            : this( capacity.GetValueOrDefault( DEFAULT_CAPACITY ), comparer )
        {
            if ( seq != null )
            {
        ...
        [M(O.AggressiveInlining), TP(REASON)] private Set() { }
        [M(O.AggressiveInlining), TP(REASON)] public static Set< T > CreateWithCloserCapacity( int capacity, IEqualityComparer< T > comparer = null )
        {
        ...
        [M(O.AggressiveInlining)] public bool Add( T value )
        {
        ...
        [M(O.AggressiveInlining)] public bool AddByRef( in T value )
        {
            #region [.try find exists.]
            var hash   = InternalGetHashCodeByRef( in value );
            var bucket = hash % _Buckets.Length;
            for ( var i = _Buckets[ bucket ] - 1; 0 <= i; /*i = _Slots[ i ].next*/ )
            {
                ref readonly var slot = ref _Slots[ i ];
        ...
        [M(O.AggressiveInlining), TP(REASON)] public bool Contains( T value )
        {
            var hash = InternalGetHashCode( value );
        ...
        [M(O.AggressiveInlining)] public bool TryGetValue( T value, out T existsValue )
        {
            var hash = InternalGetHashCode( value );
            for ( var i = _Buckets[ hash % _Buckets.Length ] - 1; 0 <= i; /*i = _Slots[ i ].next*/ )
            {
        ...
        [M(O.AggressiveInlining)] private int InternalGetHashCode( T value ) => (_Comparer.GetHashCode( value ) & 0x7FFFFFFF); //((value != null) ? (_Comparer.GetHashCode(value ) & 0x7FFFFFFF) : 0);
        [M(O.AggressiveInlining)] private int InternalGetHashCodeByRef( in T value ) => (_Comparer.GetHashCode( value ) & 0x7FFFFFFF);
